...
fn build_ranger_op(size: usize, alpha: f32, dialect: Dialect) -> Result<KernelSrc, IRError> {
    ...
    unsafe { pntwise.lower("ranger".to_string(), dialect) }
}
...
impl<G: Gpu, S: OptimiserState<G>> OptimiserState<G> for RangerLookahead<G, S> {
    ...
    fn new(device: &Arc<Device<G>>, size: usize, params: Self::Params) -> Result<Self, G::Error> {
        Ok(Self {
            op: build_ranger_op(size, params.alpha, device.dialect()).unwrap().compile(device.clone())?,
    ...
    fn set_params(&mut self, params: Self::Params) -> Result<(), G::Error> {
        ...
        self.op = build_ranger_op(self.slow_params.size(), params.alpha, device.dialect()).unwrap().compile(device)?;
        self.k = params.k;
        self.step = 0;
        Ok(())
    }
