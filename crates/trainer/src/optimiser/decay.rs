...
fn build_decay_op(size: usize, decay: f32, dialect: Dialect) -> Result<KernelSrc, IRError> {
    ...
    unsafe { pntwise.lower("decay".to_string(), dialect) }
}
...
impl<G: Gpu, S: OptimiserState<G>> OptimiserState<G> for WeightDecay<G, S> {
    ...
    fn new(device: &Arc<Device<G>>, size: usize, params: Self::Params) -> Result<Self, G::Error> {
        Ok(Self {
            op: build_decay_op(size, params.decay, device.dialect()).unwrap().compile(device.clone())?,
    ...
    fn set_params(&mut self, params: Self::Params) -> Result<(), G::Error> {
        ...
        self.op = build_decay_op(self.size, params.decay, self.device.dialect()).unwrap().compile(self.device.clone())?;
        Ok(())
    }
