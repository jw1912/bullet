use std::rc::Rc;

use crate::{
    core::DTypeTensor,
    ir::{
        IR, IRTrace,
        graph::{
            IrNodeId, IrOperationId, IrOperationType,
            operation::{Constant, ScalarConstant},
        },
        transform::{IrTransform, canonicalise::*, destructive::*, eliminate::*, foldrules::*},
    },
};

#[derive(Clone, Debug, Default)]
pub struct SimplifyPass {
    folds: Vec<Rc<dyn FoldRule>>,
    destructives: Vec<Rc<dyn DestructiveRule>>,
}

impl SimplifyPass {
    pub fn all() -> Self {
        Self::default()
            .add_fold(FoldScalarConstIntoUnary)
            .add_fold(FoldScalarConstLhsIntoBinary)
            .add_fold(FoldScalarConstRhsIntoBinary)
            .add_fold(FoldFixedSizeScalarConst)
            .add_fold(FoldVarSizeScalarConst)
            .add_fold(FoldConstIdentities)
            .add_fold(FoldMulByZero)
            .add_fold(FoldUnaryUnaryIntoUnary)
            .add_fold(FoldMulXAddMulX)
            .add_fold(FoldXAddMulX)
            .add_destructive(BroadcastUnaryIntoUnaryBroadcast)
            .add_destructive(BroadcastBinaryIntoBinaryBroadcast)
    }

    pub fn add_fold(mut self, fold: impl FoldRule) -> Self {
        self.folds.push(Rc::new(fold));
        self
    }

    pub fn add_destructive(mut self, destructive: impl DestructiveRule) -> Self {
        self.destructives.push(Rc::new(destructive));
        self
    }

    pub fn apply_single_fold(&self, ir: &mut IR, mut id: IrOperationId) -> Result<bool, IRTrace> {
        let mut success = false;

        'fold: loop {
            let op = ir.get_op(id)?;

            if let Some(consts) = Self::fold_constant_expression(ir, op.inputs(), op.op())? {
                let outputs = op.outputs().to_vec();
                for (old, new_const) in outputs.into_iter().zip(consts) {
                    let new = ir.add_op([], Ok::<_, IRTrace>(new_const))?[0];
                    ir.swap_outputs(old, new)?;
                }

                ir.remove_op(id)?;
                return Ok(true);
            }

            for fold in &self.folds {
                if let Some(new) = fold.fold(ir, op)? {
                    id = ir.replace_op(op.id(), new)?;
                    success = true;
                    continue 'fold;
                }
            }

            let op = op.clone();
            for destructive in &self.destructives {
                if destructive.apply(ir, op.clone())? {
                    return Ok(true);
                }
            }

            return Ok(success);
        }
    }

    pub fn fold_constant_expression(
        ir: &IR,
        inputs: &[IrNodeId],
        op: &Rc<dyn IrOperationType>,
    ) -> Result<Option<Vec<Constant>>, IRTrace> {
        if !inputs.is_empty() {
            let mut consts = Vec::new();

            for &input in inputs {
                let parent = ir.get_op(ir.get_parent_op(input)?)?;

                if let Some(Constant(value)) = parent.downcast().cloned() {
                    consts.push(value);
                } else if let Some(Some(scalar)) = parent.downcast().map(ScalarConstant::to_tensor) {
                    consts.push(scalar);
                } else {
                    return Ok(None);
                }
            }

            let mut tensors = Vec::new();
            for &ty in &op.outputs() {
                if let Some(size) = ty.size().evaluate_constant() {
                    tensors.push(DTypeTensor::new(ty.dtype(), size));
                } else {
                    return Ok(None);
                }
            }

            let inputs = consts.iter().collect::<Vec<_>>();
            let mut outputs = tensors.iter_mut().collect::<Vec<_>>();

            op.evaluate(&inputs, &mut outputs);

            return Ok(Some(tensors.into_iter().map(Constant).collect()));
        }

        Ok(None)
    }
}

impl IrTransform for SimplifyPass {
    fn apply(&self, ir: &mut IR) -> Result<(), IRTrace> {
        let mut success = true;

        while success {
            ir.transform(CanonicaliseCommutativeInputs)?;
            ir.transform(EliminateCopies)?;
            ir.transform(EliminateUnusedOperations)?;
            ir.transform(EliminateCommonSubExpressions)?;

            success = false;

            for op in ir.ordered_operations()? {
                success |= self.apply_single_fold(ir, op.id())?;
            }
        }

        Ok(())
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    use crate::{
        core::{Binary, DType, DTypeValue, Size, Unary},
        ir::graph::{
            IrType,
            operation::{BroadcastAcrossDimension, ScalarConstant},
        },
    };

    #[test]
    fn test_complex() -> Result<(), IRTrace> {
        let mut ir = IR::default();

        let size = Size::variable();

        let a = ir.add_const(DTypeValue::I32(1).into());
        let ba = BroadcastAcrossDimension::new(DType::I32, [1], 0, size);
        let a = ir.add_op([a], ba)?[0];

        let b = ir.add_input(IrType::new(1, DType::F32));
        let bb = BroadcastAcrossDimension::new(DType::F32, [1], 0, size);
        let b = ir.add_op([b], bb.clone())?[0];

        let c = ir.add_unary(a, Unary::Cast(DType::F32))?;
        let d = ir.add_binary(c, b, Binary::Add)?;

        let neg = Unary::BinaryWithConst { op: Binary::Mul, val: (-1.0).into(), lhs: true };
        let neg_d = ir.add_unary(d, neg)?;
        let e = ir.add_binary(neg_d, d, Binary::Add)?;

        ir.register_output(d);
        ir.register_output(e);

        ir.transform(SimplifyPass::all())?;

        assert_eq!(ir.parent_op(d)?, Some(&bb?), "{ir}");
        assert_eq!(ir.parent_op(e)?, Some(&ScalarConstant(0.0.into(), size)), "{ir}");

        ir.check_valid()
    }
}
