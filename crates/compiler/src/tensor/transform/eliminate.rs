use crate::tensor::{IRTrace, TensorIR, operation::CopyOp, transform::IRTransform};
...
impl IRTransform for EliminateCopies {
    fn apply(&self, ir: &mut TensorIR) -> Result<(), IRTrace> {
        for op in ir.ordered_operations()? {
            ...
            if op.data().downcast::<CopyOp>().is_some()
                && let [input] = op.inputs()[..]
                && let [output] = op.outputs()[..]
            {
                ...
                if !ir.is_output(input) && !ir.is_input(input)? {
                    ...
                    ir.ir_mut().replace_input_no_cycle_check(output, input)?;
...
#[cfg(test)]
mod tests {
    use crate::tensor::{DType, TType, TValue, operation::CABinary};
    ...
    fn eliminate_input_copy() -> Result<(), IRTrace> {
        ...
        assert_eq!(op.data().downcast(), Some(&CopyOp(ty)));
