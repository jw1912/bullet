...
impl IRTransform for AddOperation {
    fn apply(&self, ir: &mut IR) -> Result<(), IRTrace> {
        *self.outputs.borrow_mut() = ir.graph_mut().add_op_dyn(&self.inputs, self.operation.clone()?)?;
        Ok(())
    }
}
...
impl IRTransform for RemoveOperation {
    fn apply(&self, ir: &mut IR) -> Result<(), IRTrace> {
        ir.graph_mut().remove_op(self.0).map_err(IRTrace::Root)
    }
}
...
impl IRTransform for SwapOutputs {
    fn apply(&self, ir: &mut IR) -> Result<(), IRTrace> {
        ...
        ir.graph_mut().swap_outputs_unchecked(self.0, self.1)?;
        ir.check_valid()
    }
}
...
impl IRTransform for ReplaceInput {
    fn apply(&self, ir: &mut IR) -> Result<(), IRTrace> {
        ...
        ir.graph_mut().replace_input_unchecked(self.new, self.old)?;
        ir.check_valid()
    }
}
