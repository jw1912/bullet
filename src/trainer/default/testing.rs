...
pub struct TestSettings<'a, T: EngineType> {
    ...
    pub out_dir: &'a str,
    /// Path to fastchess executable.
    pub fastchess_path: &'a str,
...
impl<T: EngineType> TestSettings<'_, T> {
    pub fn setup<LR: LrScheduler, WDL: WdlScheduler>(&self, schedule: &TrainingSchedule<LR, WDL>) {
        let output = fastchess::FastChessCommand::health_check(self.fastchess_path);

        assert!(output.status.success(), "Could not start fastchess!");
    ...
    pub fn dispatch(&self, net_id: &str, superbatch: usize) -> JoinHandle<()> {
        ...
        let args = FastChessArgs {
            fastchess_path: self.fastchess_path.to_string(),
            dev_engine_path,
            base_engine_path,
        ...
        thread::spawn(move || {
            let (elo, err) = fastchess::run_games(args).unwrap();
            let mut file =
